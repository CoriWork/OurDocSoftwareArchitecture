...
const MainPage: React.FC = () => {
  ...
  const groupRoomsByUser = (rooms: Room[]): SiderMenuItem[] => {
    ...
    map.forEach((rooms, username) => {
      ...
      items.push(
        getItem(username, `user-${username}`, <UserOutlined />, children)
      );
    });
    return items;
  };
  ...
  const mapRoomsToMenuItems = (rooms: Room[]): SiderMenuItem[] => {
    return rooms.map((room) =>
      getItem(room.room_name, `${room.room_id}`, <FileOutlined />)
    );
  };
  ...
  const filteredItems = React.useMemo(() => {
    ...
    return filterMenu(
      mode === "user" ? groupRoomsByUser(rooms) : mapRoomsToMenuItems(rooms)
    );
